...
export default function GalleryClient({ type, pictures }: GalleryClientProps) {
  ...
  return (
    <>
      {type === "article" && (
        <div className="grid grid-cols-2 md:grid-cols-4 auto-rows-[160px] gap-4 auto-flow-dense">
          {pictures.map((picture, i) => {
            const extraClass = i % 6 === 0 ? "col-span-2 row-span-2" : "";
      ...
      {type === "gallery" && (
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 auto-flow-dense">
          {pictures.map((picture, i) => {
            const extraClass = i % 7 === 0 ? "row-span-2" : "";
            ...
            return (
              <button
                ...
                className={`group overflow-hidden rounded-lg shadow-lg ring-1 ring-white/10
                  transition-transform duration-300 ease-out hover:scale-105 cursor-pointer ${extraClass}`}
